...
function MainApp() {
  ...
  return <div className="flex flex-col h-screen bg-surface-base overflow-hidden text-ink-primary pb-safe"><header className="w-full bg-surface-elevated/85 backdrop-blur-md border-b border-line/60 rounded-b-3xl shadow-surface-lg z-20 px-5 flex items-center justify-between" style={{ paddingTop: 'calc(env(safe-area-inset-top, 0px) + 16px)', paddingBottom: '16px' }}><div className="flex items-center gap-3"><MatrixLogo activeTab={activeTab} /><AnimatePresence mode="wait"><motion.h1 key={screenLabel} initial={{ opacity: 0, y: -6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 6 }} transition={{ duration: 0.18 }} className="text-title font-semibold text-ink-primary tracking-tight">{screenLabel}</motion.h1></AnimatePresence></div><div className="w-2 h-2 rounded-full bg-accent" aria-hidden="true" /></header>
